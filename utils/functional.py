...
def get_zero_plane(height: float, width: float, dtype: torch.dtype, device: torch.device):
    return torch.tensor([[
        [0.0, 0.0, 0.0],
...
def accele_from_dof(dof, height=None, width=None):
    old_type = dof.dtype
    dof = dof.type(torch.float64)
    ...
    mat = matrix_inv[:-1] + matrix[1:] - 2 * torch.eye(4, dtype=dof.dtype, device=dof.device).unsqueeze(0)

    if height is not None and width is not None:
        zero_plane = get_zero_plane(height=height, width=width, dtype=dof.dtype, device=dof.device)
        zero_plane = F.pad(zero_plane, (0, 1))
        zero_plane[:, :, 3] = 1
        zero_plane = zero_plane.expand(len(dof) - 1, 3, 4).permute(0, 2, 1)
        accele = torch.bmm(mat, zero_plane).permute(0, 2, 1)[:, :, :3]
    else:
        accele = mat[:, :3, 3]

    accele = accele.type(old_type)
    return accele
